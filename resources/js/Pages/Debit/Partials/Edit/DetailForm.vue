...
<template>
  <form-section @submitted="updateDebitDetail">
    <template #title>
      <p class="dark:text-slate-50 ">Edit Debit (Akandikwereta)</p>
    </template>
    ...
    <template #form>
      ...
      <div class="col-span-6 sm:col-span-4">
        <div class="flex">
          ...
          <div :class="{ [`hidden`]: form.currency == 'usd' }">
        ...
        <p
          ...
          :class="{ [`hidden`]: form.currency == 'usd' }"
        >
...
const form = useForm({
  date: props.debit.date,
  source: props.debit.source_id,
  title: props.debit.title, 
  price: props.debit.price,
  detail: pointConverter(props.debit.detail),
  currency: props.debit.currency,
  rate: props.debit.rate,
  remember: true,
});
...
function usdEquivalent() {
  ...
  return "$" + (form.price / form.rate).toFixed(2);
}


const updateDebitDetail = () => {
    form.put(route("debits.update", props.debit.uuid), {
      errorBag: "updateDebitDetail",
      preserveScroll: true,
    });
  };
</script>
