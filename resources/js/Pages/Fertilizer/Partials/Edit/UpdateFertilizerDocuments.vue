...
<template>
  <form-section @submitted="updateFertilizerDetail">
    <template #title> <p class="text-slate-100">  Update Fertilizer Documents</p> </template>

    <template #description>
      <p class="text-slate-50">  Upload Fertilizer related documents eg. User Manuals</p>
  ...
  <edit-document-list :documents="fertilizer.documents" path="attachments.show" />
  <edit-document-list
    :documents="fertilizer.trashed_documents"
...
const form = useForm({
  title: props.fertilizer.title,
  images: null,
  detail: pointConverter(props.fertilizer.detail),
  category: null,
  fertilizer_uuid: props.fertilizer.uuid,
  type: props.fertilizer.type,
  document_title: null,
  document: null,
  remember: true,
  _method: "PUT",
...
const updateFertilizerDetail = () => {
  form.post(route("fertilizers.update", props.fertilizer.uuid), {
    errorBag: "updateFertilizerDetail",
    preserveScroll: true,
  });
};
</script>
