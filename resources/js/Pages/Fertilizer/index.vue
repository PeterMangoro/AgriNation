  ...
  <app-layout title="Fertilizers">
    ...
    <div class="mx-auto max-w-7xl sm:px-6 lg:px-8">
      ...
      <div class="flex flex-wrap justify-between pb-2">
        ...
        <span class="my-auto">
          <!-- <button-link
              :link="route('fertilizers.trashed.index')"
              class=" bg-slate-600"
              >Deleted Fertilizers</button-link
            > -->
        </span>
      </div>

      <Table
        heading="Fertilizer Table"
        :pagination="data.fertilizers.links"
        path="fertilizers.create"
        button="Add new Fertilizer"
      >
        <template #tableHead>
        ...
        <TableRow
          v-for="fertilizer in data.fertilizers.data"
          :key="fertilizer.id"
        >
          <TableData>{{ fertilizer.title }}</TableData>
          <TableData class="hidden sm:table-cell">{{
            fertilizer.quantity
          }}</TableData>
          <TableData class="w-auto hidden sm:table-cell">
            <p
              v-for="(point, index) in fertilizer.detail"
              :key="index"
          ...
          >
          <!-- <TableData class="hidden sm:table-cell"
              ><img class="w-20 h-20 rounded" :src="fertilizer.latest_image"
            /></TableData> -->

          <TableData class="hidden sm:table-cell">
            {{ fertilizer.price }} <br>
            <span v-if="fertilizer.usd_equivalent" class="font-bold"> ({{ fertilizer.usd_equivalent }})</span>

          </TableData>
          <!-- <TableData class="hidden sm:table-cell">
              <span v-if="fertilizer.status == 'For Sale'">Available</span>
          ...
          <TableData>
            <div class="flex space-x-1">
              <ButtonLink
                class=""
                :link="route('fertilizers.edit', fertilizer.uuid)"
                >View</ButtonLink
              >
              <Button
                ...
                @click="show_delete_confirmation(fertilizer.id)"
                as="button"
                type="button"
                >Suspend</Button
              >
      ...
      <span class="w-full" v-if="data.fertilizers.data.length == 0">
        <no-result-display />
      </span>
    ...
    <div v-if="show">
      <delete-confirmation
        :message="message"
        path="fertilizers.destroy"
        :item="fertilizer_to_be_deleted"
        @close="show = false"
      />
    </div>
  </app-layout>
...
const message =
  "Are you sure you want to delete this fertilizer. Deleted fertilizers will not be seen by customers, but can be found in the trash if you want to restore them";

const show = ref(false);
const fertilizer_to_be_deleted = ref(null);

function show_delete_confirmation(fertilizer) {
  show.value = true;
  fertilizer_to_be_deleted.value = fertilizer;
}

function sort(column) {
  useSort(column, "fertilizers.index");
}
</script>
