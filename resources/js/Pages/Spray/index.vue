  ...
  <app-layout title="Sprays">
    ...
    <div class="mx-auto max-w-7xl sm:px-6 lg:px-8">
      ...
      <div class="flex flex-wrap justify-between pb-2">
        ...
        <span class="my-auto">
          <!-- <button-link
              :link="route('sprays.trashed.index')"
              class=" bg-slate-600"
              >Deleted Sprays</button-link
            > -->
        </span>
      </div>
...
- {{ location.title  }}
          ...
          <TableData>
            <div class="flex space-x-1">
              <ButtonLink
                class=""
                :link="route('sprays.edit', spray.id)"
                >View</ButtonLink
              >
              <Button
                ...
                @click="show_delete_confirmation(spray.id)"
                as="button"
                type="button"
                >Suspend</Button
              >
      ...
      <span class="w-full" v-if="data.sprays.data.length == 0">
        <no-result-display />
      </span>
...
const message =
  "Are you sure you want to delete this spray. Deleted sprays will not be seen by customers, but can be found in the trash if you want to restore them";

const show = ref(false);
const spray_to_be_deleted = ref(null);

function show_delete_confirmation(spray) {
  show.value = true;
  spray_to_be_deleted.value = spray;
}

function sort(column) {
  useSort(column, "sprays.index");
}
</script>
